...
var STACK_BASE = 17338496, STACKTOP = STACK_BASE, STACK_MAX = 22581376, DYNAMIC_BASE = 22581376, DYNAMICTOP_PTR = 17337504;
...
var INITIAL_TOTAL_MEMORY = Module["TOTAL_MEMORY"] || 629145600;
...
var wasmBinaryFile = "CirclePong.wasm";
...
function _emscripten_asm_const_sync_on_main_thread_i(code) {
 if (ENVIRONMENT_IS_PTHREAD) {
  return _emscripten_proxy_to_main_thread_js(-1 - code, 1);
 }
...
if (!ENVIRONMENT_IS_PTHREAD) {
 memoryInitializer = "CirclePong.js.mem";
}

var tempDoublePtr;

if (!ENVIRONMENT_IS_PTHREAD) tempDoublePtr = 17338480;
...
var __main_thread_futex_wait_address = 17338464;
...
var PThread = {
 ...
 initMainThreadBlock: function() {
  ...
  PThread.mainThreadBlock = 17337680;
 ...
 createNewWorkers: function(numWorkers) {
  if (typeof SharedArrayBuffer === "undefined") return [];
  var pthreadMainJs = "CirclePong.worker.js";
...
function _alBufferData(bufferId, format, pData, size, freq) {
 if (ENVIRONMENT_IS_PTHREAD) return _emscripten_proxy_to_main_thread_js(25, 1, bufferId, format, pData, size, freq);
 if (!AL.currentCtx) {
  return;
 }
 var buf = AL.buffers[bufferId];
...
function _alDeleteBuffers(count, pBufferIds) {
 if (ENVIRONMENT_IS_PTHREAD) return _emscripten_proxy_to_main_thread_js(26, 1, count, pBufferIds);
 if (!AL.currentCtx) {
  return;
 }
...
function _alSourcei(sourceId, param, value) {
 if (ENVIRONMENT_IS_PTHREAD) return _emscripten_proxy_to_main_thread_js(27, 1, sourceId, param, value);
 switch (param) {
 case 514:
 case 4097:
 case 4098:
...
function _alDeleteSources(count, pSourceIds) {
 if (ENVIRONMENT_IS_PTHREAD) return _emscripten_proxy_to_main_thread_js(28, 1, count, pSourceIds);
 if (!AL.currentCtx) {
  return;
 }
 for (var i = 0; i < count; ++i) {
...
function _alDistanceModel(model) {
 if (ENVIRONMENT_IS_PTHREAD) return _emscripten_proxy_to_main_thread_js(29, 1, model);
...
function _alGetSourcei(sourceId, param, pValue) {
 if (ENVIRONMENT_IS_PTHREAD) return _emscripten_proxy_to_main_thread_js(33, 1, sourceId, param, pValue);
...
function _alSourcePlay(sourceId) {
 if (ENVIRONMENT_IS_PTHREAD) return _emscripten_proxy_to_main_thread_js(36, 1, sourceId);
 if (!AL.currentCtx) {
  return;
 }
 var src = AL.currentCtx.sources[sourceId];
...
function _alSourceQueueBuffers(sourceId, count, pBufferIds) {
 if (ENVIRONMENT_IS_PTHREAD) return _emscripten_proxy_to_main_thread_js(37, 1, sourceId, count, pBufferIds);
 if (!AL.currentCtx) {
  return;
 }
 var src = AL.currentCtx.sources[sourceId];
...
function _alSourceStop(sourceId) {
 if (ENVIRONMENT_IS_PTHREAD) return _emscripten_proxy_to_main_thread_js(38, 1, sourceId);
 if (!AL.currentCtx) {
  return;
 }
 var src = AL.currentCtx.sources[sourceId];
...
function _alSourceUnqueueBuffers(sourceId, count, pBufferIds) {
 if (ENVIRONMENT_IS_PTHREAD) return _emscripten_proxy_to_main_thread_js(39, 1, sourceId, count, pBufferIds);
 if (!AL.currentCtx) {
  return;
 }
 var src = AL.currentCtx.sources[sourceId];
...
function _alSourcef(sourceId, param, value) {
 if (ENVIRONMENT_IS_PTHREAD) return _emscripten_proxy_to_main_thread_js(40, 1, sourceId, param, value);
 switch (param) {
 case 4097:
 case 4098:
 case 4099:
...
function _alSourcefv(sourceId, param, pValues) {
 if (ENVIRONMENT_IS_PTHREAD) return _emscripten_proxy_to_main_thread_js(41, 1, sourceId, param, pValues);
 if (!AL.currentCtx) {
  return;
 }
 if (!pValues) {
...
function _alcCloseDevice(deviceId) {
 if (ENVIRONMENT_IS_PTHREAD) return _emscripten_proxy_to_main_thread_js(42, 1, deviceId);
...
function _alcCreateContext(deviceId, pAttrList) {
 if (ENVIRONMENT_IS_PTHREAD) return _emscripten_proxy_to_main_thread_js(43, 1, deviceId, pAttrList);
...
function _alcDestroyContext(contextId) {
 if (ENVIRONMENT_IS_PTHREAD) return _emscripten_proxy_to_main_thread_js(44, 1, contextId);
...
function _alcGetString(deviceId, param) {
 if (ENVIRONMENT_IS_PTHREAD) return _emscripten_proxy_to_main_thread_js(45, 1, deviceId, param);
...
function _alcMakeContextCurrent(contextId) {
 if (ENVIRONMENT_IS_PTHREAD) return _emscripten_proxy_to_main_thread_js(46, 1, contextId);
 if (contextId === 0) {
  AL.currentCtx = null;
  return 0;
 } else {
...
function _alcOpenDevice(pDeviceName) {
 if (ENVIRONMENT_IS_PTHREAD) return _emscripten_proxy_to_main_thread_js(47, 1, pDeviceName);
 if (pDeviceName) {
...
function _emscripten_enter_soft_fullscreen(target, fullscreenStrategy) {
 if (ENVIRONMENT_IS_PTHREAD) return _emscripten_proxy_to_main_thread_js(50, 1, target, fullscreenStrategy);
 if (!target) target = "#canvas";
...
function _emscripten_get_fullscreen_status(fullscreenStatus) {
 if (ENVIRONMENT_IS_PTHREAD) return _emscripten_proxy_to_main_thread_js(53, 1, fullscreenStatus);
...
function _emscripten_get_gamepad_status(index, gamepadState) {
 if (ENVIRONMENT_IS_PTHREAD) return _emscripten_proxy_to_main_thread_js(54, 1, index, gamepadState);
...
function _emscripten_get_pointerlock_status(pointerlockStatus) {
 if (ENVIRONMENT_IS_PTHREAD) return _emscripten_proxy_to_main_thread_js(56, 1, pointerlockStatus);
...
function _emscripten_request_fullscreen_strategy(target, deferUntilInEventHandler, fullscreenStrategy) {
 if (ENVIRONMENT_IS_PTHREAD) return _emscripten_proxy_to_main_thread_js(57, 1, target, deferUntilInEventHandler, fullscreenStrategy);
 var strategy = {};
 strategy.scaleMode = HEAP32[fullscreenStrategy >> 2];
 strategy.canvasResolutionScaleMode = HEAP32[fullscreenStrategy + 4 >> 2];
 strategy.filteringMode = HEAP32[fullscreenStrategy + 8 >> 2];
...
function _emscripten_request_pointerlock(target, deferUntilInEventHandler) {
 if (ENVIRONMENT_IS_PTHREAD) return _emscripten_proxy_to_main_thread_js(58, 1, target, deferUntilInEventHandler);
 if (!target) target = "#canvas";
 target = __findEventTarget(target);
 if (!target) return -4;
...
function _emscripten_set_beforeunload_callback_on_thread(userData, callbackfunc, targetThread) {
 if (ENVIRONMENT_IS_PTHREAD) return _emscripten_proxy_to_main_thread_js(60, 1, userData, callbackfunc, targetThread);
...
function _emscripten_webgl_destroy_context_main_thread(contextHandle) {
 if (ENVIRONMENT_IS_PTHREAD) return _emscripten_proxy_to_main_thread_js(72, 1, contextHandle);
 return _emscripten_webgl_destroy_context_calling_thread(contextHandle);
}
...
function _getaddrinfo(node, service, hint, out) {
 if (ENVIRONMENT_IS_PTHREAD) return _emscripten_proxy_to_main_thread_js(73, 1, node, service, hint, out);
 var addr = 0;
 var port = 0;
 var flags = 0;
 var family = 0;
...
function _getenv(name) {
 if (ENVIRONMENT_IS_PTHREAD) return _emscripten_proxy_to_main_thread_js(74, 1, name);
 if (name === 0) return 0;
 name = UTF8ToString(name);
...
function _gethostbyname(name) {
 if (ENVIRONMENT_IS_PTHREAD) return _emscripten_proxy_to_main_thread_js(75, 1, name);
 name = UTF8ToString(name);
 var ret = _malloc(20);
...
function _gethostbyaddr(addr, addrlen, type) {
 if (ENVIRONMENT_IS_PTHREAD) return _emscripten_proxy_to_main_thread_js(76, 1, addr, addrlen, type);
 if (type !== 2) {
  ___setErrNo(5);
  return null;
 }
...
var ___tm_timezone = (stringToUTF8("GMT", 17337584, 4), 17337584);
...
function _pthread_cancel(thread) {
 ...
 if (!ENVIRONMENT_IS_PTHREAD) __cancel_thread(thread); else postMessage({
  "cmd": "cancelThread",
  "thread": thread
 });
 return 0;
...
function _utime(path, times) {
 if (ENVIRONMENT_IS_PTHREAD) return _emscripten_proxy_to_main_thread_js(78, 1, path, times);
 var time;
 if (times) {
  var offset = 4;
...
function nullFunc_fif(x) {
 abortFnPtrError(x, "fif");
}

function nullFunc_fiff(x) {
 abortFnPtrError(x, "fiff");
}

function nullFunc_fiffi(x) {
 abortFnPtrError(x, "fiffi");
}

function nullFunc_fifi(x) {
 abortFnPtrError(x, "fifi");
}

function nullFunc_fifii(x) {
 abortFnPtrError(x, "fifii");
}

function nullFunc_fifiii(x) {
 abortFnPtrError(x, "fifiii");
}
...
function nullFunc_fiiiiiif(x) {
 abortFnPtrError(x, "fiiiiiif");
}

function nullFunc_fiiiiiifi(x) {
 abortFnPtrError(x, "fiiiiiifi");
}

function nullFunc_fiiiiiifiifif(x) {
 abortFnPtrError(x, "fiiiiiifiifif");
}

function nullFunc_fiiiiiifiiiif(x) {
 abortFnPtrError(x, "fiiiiiifiiiif");
}

function nullFunc_fiiiiiii(x) {
 abortFnPtrError(x, "fiiiiiii");
}

function nullFunc_fiiiiiiii(x) {
 abortFnPtrError(x, "fiiiiiiii");
}

function nullFunc_fiiijiijiijii(x) {
 abortFnPtrError(x, "fiiijiijiijii");
}

function nullFunc_fij(x) {
 abortFnPtrError(x, "fij");
}
...
function nullFunc_iidf(x) {
 abortFnPtrError(x, "iidf");
}

function nullFunc_iidfi(x) {
 abortFnPtrError(x, "iidfi");
}
...
function nullFunc_iidii(x) {
 abortFnPtrError(x, "iidii");
}

function nullFunc_iidiii(x) {
 abortFnPtrError(x, "iidiii");
}
...
function nullFunc_iiiddii(x) {
 abortFnPtrError(x, "iiiddii");
}

function nullFunc_iiidi(x) {
 abortFnPtrError(x, "iiidi");
}

function nullFunc_iiif(x) {
 abortFnPtrError(x, "iiif");
}

function nullFunc_iiiff(x) {
 abortFnPtrError(x, "iiiff");
}

function nullFunc_iiiffffiifii(x) {
 abortFnPtrError(x, "iiiffffiifii");
}

function nullFunc_iiiffi(x) {
 abortFnPtrError(x, "iiiffi");
}

function nullFunc_iiiffii(x) {
 abortFnPtrError(x, "iiiffii");
}

function nullFunc_iiifi(x) {
 abortFnPtrError(x, "iiifi");
}

function nullFunc_iiifii(x) {
 abortFnPtrError(x, "iiifii");
}

function nullFunc_iiifiifi(x) {
 abortFnPtrError(x, "iiifiifi");
}

function nullFunc_iiifiii(x) {
 abortFnPtrError(x, "iiifiii");
}

function nullFunc_iiifiiiif(x) {
 abortFnPtrError(x, "iiifiiiif");
}
...
function nullFunc_iiiiddiii(x) {
 abortFnPtrError(x, "iiiiddiii");
}

function nullFunc_iiiidi(x) {
 abortFnPtrError(x, "iiiidi");
}

function nullFunc_iiiidii(x) {
 abortFnPtrError(x, "iiiidii");
}

function nullFunc_iiiif(x) {
 abortFnPtrError(x, "iiiif");
}

function nullFunc_iiiiffffi(x) {
 abortFnPtrError(x, "iiiiffffi");
}

function nullFunc_iiiiffi(x) {
 abortFnPtrError(x, "iiiiffi");
}

function nullFunc_iiiiffii(x) {
 abortFnPtrError(x, "iiiiffii");
}
...
function nullFunc_iiiiffiiiiiii(x) {
 abortFnPtrError(x, "iiiiffiiiiiii");
}

function nullFunc_iiiifi(x) {
 abortFnPtrError(x, "iiiifi");
}

function nullFunc_iiiifii(x) {
 abortFnPtrError(x, "iiiifii");
}

function nullFunc_iiiifiii(x) {
 abortFnPtrError(x, "iiiifiii");
}

function nullFunc_iiiifiiii(x) {
 abortFnPtrError(x, "iiiifiiii");
}

function nullFunc_iiiifiiiii(x) {
 abortFnPtrError(x, "iiiifiiiii");
}
...
function nullFunc_iiiiiffii(x) {
 abortFnPtrError(x, "iiiiiffii");
}

function nullFunc_iiiiifi(x) {
 abortFnPtrError(x, "iiiiifi");
}

function nullFunc_iiiiifii(x) {
 abortFnPtrError(x, "iiiiifii");
}

function nullFunc_iiiiifiii(x) {
 abortFnPtrError(x, "iiiiifiii");
}

function nullFunc_iiiiifiiiif(x) {
 abortFnPtrError(x, "iiiiifiiiif");
}

function nullFunc_iiiiifiiiiif(x) {
 abortFnPtrError(x, "iiiiifiiiiif");
}
...
function nullFunc_iiiiiif(x) {
 abortFnPtrError(x, "iiiiiif");
}

function nullFunc_iiiiiiffii(x) {
 abortFnPtrError(x, "iiiiiiffii");
}

function nullFunc_iiiiiifi(x) {
 abortFnPtrError(x, "iiiiiifi");
}

function nullFunc_iiiiiifii(x) {
 abortFnPtrError(x, "iiiiiifii");
}

function nullFunc_iiiiiifiif(x) {
 abortFnPtrError(x, "iiiiiifiif");
}

function nullFunc_iiiiiifiii(x) {
 abortFnPtrError(x, "iiiiiifiii");
}
...
function nullFunc_iiiiiiifi(x) {
 abortFnPtrError(x, "iiiiiiifi");
}

function nullFunc_iiiiiiifii(x) {
 abortFnPtrError(x, "iiiiiiifii");
}

function nullFunc_iiiiiiifiif(x) {
 abortFnPtrError(x, "iiiiiiifiif");
}
...
function nullFunc_iijiiii(x) {
 abortFnPtrError(x, "iijiiii");
}

function nullFunc_iijj(x) {
 abortFnPtrError(x, "iijj");
}

function nullFunc_iijji(x) {
 abortFnPtrError(x, "iijji");
}

function nullFunc_iijjiii(x) {
 abortFnPtrError(x, "iijjiii");
}

function nullFunc_iijjji(x) {
 abortFnPtrError(x, "iijjji");
}

function nullFunc_ijj(x) {
 abortFnPtrError(x, "ijj");
}
...
function nullFunc_jiiiiiii(x) {
 abortFnPtrError(x, "jiiiiiii");
}

function nullFunc_jiiiiiiiii(x) {
 abortFnPtrError(x, "jiiiiiiiii");
}

function nullFunc_jiij(x) {
 abortFnPtrError(x, "jiij");
}
...
function nullFunc_vidiii(x) {
 abortFnPtrError(x, "vidiii");
}

function nullFunc_vidiiii(x) {
 abortFnPtrError(x, "vidiiii");
}

function nullFunc_vidiiiii(x) {
 abortFnPtrError(x, "vidiiiii");
}
...
function nullFunc_viidddd(x) {
 abortFnPtrError(x, "viidddd");
}

function nullFunc_viidf(x) {
 abortFnPtrError(x, "viidf");
}
...
function nullFunc_viiff(x) {
 abortFnPtrError(x, "viiff");
}

function nullFunc_viifff(x) {
 abortFnPtrError(x, "viifff");
}

function nullFunc_viiffffffffiiii(x) {
 abortFnPtrError(x, "viiffffffffiiii");
}

function nullFunc_viiffffffii(x) {
 abortFnPtrError(x, "viiffffffii");
}

function nullFunc_viiffffiiii(x) {
 abortFnPtrError(x, "viiffffiiii");
}

function nullFunc_viifffi(x) {
 abortFnPtrError(x, "viifffi");
}

function nullFunc_viiffi(x) {
 abortFnPtrError(x, "viiffi");
}

function nullFunc_viiffifi(x) {
 abortFnPtrError(x, "viiffifi");
}

function nullFunc_viiffii(x) {
 abortFnPtrError(x, "viiffii");
}

function nullFunc_viiffiiiffffi(x) {
 abortFnPtrError(x, "viiffiiiffffi");
}
...
function nullFunc_viififi(x) {
 abortFnPtrError(x, "viififi");
}

function nullFunc_viifii(x) {
 abortFnPtrError(x, "viifii");
}

function nullFunc_viifiiff(x) {
 abortFnPtrError(x, "viifiiff");
}

function nullFunc_viifiifi(x) {
 abortFnPtrError(x, "viifiifi");
}

function nullFunc_viifiii(x) {
 abortFnPtrError(x, "viifiii");
}

function nullFunc_viifiiii(x) {
 abortFnPtrError(x, "viifiiii");
}

function nullFunc_viifiiiii(x) {
 abortFnPtrError(x, "viifiiiii");
}

function nullFunc_viifiiiiiii(x) {
 abortFnPtrError(x, "viifiiiiiii");
}
...
function nullFunc_viiidddd(x) {
 abortFnPtrError(x, "viiidddd");
}

function nullFunc_viiidi(x) {
 abortFnPtrError(x, "viiidi");
}
...
function nullFunc_viiifif(x) {
 abortFnPtrError(x, "viiifif");
}

function nullFunc_viiifii(x) {
 abortFnPtrError(x, "viiifii");
}

function nullFunc_viiifiif(x) {
 abortFnPtrError(x, "viiifiif");
}

function nullFunc_viiifiiffii(x) {
 abortFnPtrError(x, "viiifiiffii");
}

function nullFunc_viiifiiiii(x) {
 abortFnPtrError(x, "viiifiiiii");
}
...
function nullFunc_viiiidii(x) {
 abortFnPtrError(x, "viiiidii");
}
...
function nullFunc_viiiiffi(x) {
 abortFnPtrError(x, "viiiiffi");
}

function nullFunc_viiiifi(x) {
 abortFnPtrError(x, "viiiifi");
}
...
function nullFunc_viiiifiif(x) {
 abortFnPtrError(x, "viiiifiif");
}

function nullFunc_viiiifiiiiif(x) {
 abortFnPtrError(x, "viiiifiiiiif");
}
...
function nullFunc_viiiiiff(x) {
 abortFnPtrError(x, "viiiiiff");
}

function nullFunc_viiiiiffi(x) {
 abortFnPtrError(x, "viiiiiffi");
}
...
function nullFunc_viiiiifii(x) {
 abortFnPtrError(x, "viiiiifii");
}

function nullFunc_viiiiifiiiif(x) {
 abortFnPtrError(x, "viiiiifiiiif");
}
...
function nullFunc_viiiiiiiiiiifii(x) {
 abortFnPtrError(x, "viiiiiiiiiiifii");
}
...
function nullFunc_viiiij(x) {
 abortFnPtrError(x, "viiiij");
}

function nullFunc_viiij(x) {
 abortFnPtrError(x, "viiij");
}

function nullFunc_viiijji(x) {
 abortFnPtrError(x, "viiijji");
}
...
function nullFunc_viijijj(x) {
 abortFnPtrError(x, "viijijj");
}

function nullFunc_viijj(x) {
 abortFnPtrError(x, "viijj");
}

function nullFunc_viijjjii(x) {
 abortFnPtrError(x, "viijjjii");
}
...
function nullFunc_viji(x) {
 abortFnPtrError(x, "viji");
}

function nullFunc_vijiiii(x) {
 abortFnPtrError(x, "vijiiii");
}

function nullFunc_vijji(x) {
 abortFnPtrError(x, "vijji");
}

function nullFunc_vijjjii(x) {
 abortFnPtrError(x, "vijjjii");
}

function nullFunc_vj(x) {
 abortFnPtrError(x, "vj");
}

function nullFunc_vjfi(x) {
 abortFnPtrError(x, "vjfi");
}
...
var asmLibraryArg = {
 ...
 "__cancel_thread": __cancel_thread,
 ...
 "__emscripten_proxied_gl_context_activated_from_main_browser_thread": __emscripten_proxied_gl_context_activated_from_main_browser_thread,
 ...
 "_emscripten_webgl_destroy_context_main_thread": _emscripten_webgl_destroy_context_main_thread,
 ...
 "_emscripten_webgl_make_context_current_calling_thread": _emscripten_webgl_make_context_current_calling_thread,
 "_exit": _exit,
 "_fd_close": _fd_close,
 "_fd_read": _fd_read,
 "_fd_seek": _fd_seek,
 ...
 "_pthread_setschedparam": _pthread_setschedparam,
 ...
 "nullFunc_iiiiddiii": nullFunc_iiiiddiii,
 ...
 "nullFunc_viiiidii": nullFunc_viiiidii,
...
var _emscripten_webgl_commit_frame = Module["_emscripten_webgl_commit_frame"] = function() {
 assert(runtimeInitialized, "you need to wait for the runtime to be ready (e.g. wait for main() to be called)");
 assert(!runtimeExited, "the runtime was exited (use NO_EXIT_RUNTIME to keep it alive after main() exits)");
 return Module["asm"]["_emscripten_webgl_commit_frame"].apply(null, arguments);
};

var _emscripten_webgl_get_current_context = Module["_emscripten_webgl_get_current_context"] = function() {
 assert(runtimeInitialized, "you need to wait for the runtime to be ready (e.g. wait for main() to be called)");
 assert(!runtimeExited, "the runtime was exited (use NO_EXIT_RUNTIME to keep it alive after main() exits)");
 return Module["asm"]["_emscripten_webgl_get_current_context"].apply(null, arguments);
};

var _emscripten_webgl_make_context_current = Module["_emscripten_webgl_make_context_current"] = function() {
 assert(runtimeInitialized, "you need to wait for the runtime to be ready (e.g. wait for main() to be called)");
 assert(!runtimeExited, "the runtime was exited (use NO_EXIT_RUNTIME to keep it alive after main() exits)");
 return Module["asm"]["_emscripten_webgl_make_context_current"].apply(null, arguments);
};
...
var dynCall_iiiiddiii = Module["dynCall_iiiiddiii"] = function() {
 assert(runtimeInitialized, "you need to wait for the runtime to be ready (e.g. wait for main() to be called)");
 assert(!runtimeExited, "the runtime was exited (use NO_EXIT_RUNTIME to keep it alive after main() exits)");
 return Module["asm"]["dynCall_iiiiddiii"].apply(null, arguments);
};
